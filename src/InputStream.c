...
// RePc: Configurable mouse batching interval (default 1ms)
static int mouseBatchingIntervalMs = 1;
...
static void inputSendThreadProc(void* context) {
    ...
    while (!PltIsThreadInterrupted(&inputSendThread)) {
        ...
        // If it's a relative mouse move packet, we can also do batching
            ...
            // Delay for batching if required
            if (now < lastMousePacketTime + mouseBatchingIntervalMs) {
                flushInputOnControlStream();
                PltSleepMs((int)(lastMousePacketTime + mouseBatchingIntervalMs - now));
                now = PltGetMillis();
            }
...
int LiSendMouseMoveEvent(short deltaX, short deltaY) {
    ...
    if (!currentRelativeMouseState.dirty) {
        ...
        // RePc: Send relative mouse moves as unreliable when low-latency input is negotiated.
        // This is safe because deltas accumulate — a lost packet is compensated by the next one.
        if (RepcFeaturesEnabled & REPC_FF_LOW_LATENCY_INPUT) {
...
int LiSendMousePositionEvent(short x, short y, short referenceWidth, short referenceHeight) {
    ...
    if (!currentAbsoluteMouseState.dirty) {
        ...
        // RePc: Send absolute mouse moves as unreliable when low-latency input is negotiated.
        // Only the latest position matters — a lost packet is replaced by the next update.
        if (RepcFeaturesEnabled & REPC_FF_LOW_LATENCY_INPUT) {
            holder->enetPacketFlags = ENET_PACKET_FLAG_UNSEQUENCED;
        }
        else {
